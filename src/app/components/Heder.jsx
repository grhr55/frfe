...
export default function FullStackPortfolio() {
  ...
  const fetchProducts = async () => {
    try {
      const res = await fetch("https://iefhie-zw3s.onrender.com/portfol/porf");
...
const fetchReactions = async (productId) => {
  try {
    ...
    const res = await fetch(`https://iefhie-zw3s.onrender.com/likos/lice/${productId}`);
...
const sendDeviceReaction = async ({ productId, deviceLike = 0, deviceDislike = 0, views = 0 }) => {
  try {
    await fetch("https://iefhie-zw3s.onrender.com/likos/reaction", { 
      method: "POST",
...
>
 ...
 <div className="w-full h-44 sm:h-52 md:h-56 lg:h-60 mb-4 overflow-hidden rounded-2xl border border-white/30 shadow-xl relative">
    ...
    <Image
    src={`https://iefhie-zw3s.onrender.com/portfol${product.img}`}
